...
from nlsam.block_utils import im2col_nd
...
def local_piesno(data, N, size=5, return_mask=True):
    ...
    block_shape = (size, size, size, data.shape[-1])

    reshaped_maps = im2col_nd(data, block_shape, block_shape, reshape_2D=False).reshape((-1,) + block_shape)
